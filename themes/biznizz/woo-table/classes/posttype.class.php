...
-----------------------------------------------------------------------------------*/
	...
	class WooTable_PostType_Reservation {
		...
		function init () {
			...
			$this->singular_label = 'Appointment';
			...
			register_post_type( $this->token,
				array(
					'labels' => array(
						...
						'not_found' => __( '<br><br>No future ' . strtolower( $this->singular_label ) . 's have been scheduled.<br><br><br>', 'woothemes' ),
		...
		public function meta_box_reservationdata () {
			...
			$services_other = get_post_meta($post->ID, 'services_other', true);
			...
			$box_content .= '<span style="visiblity:hidden;" class="right"><input type="text" name="number_of_people" id="number_of_people" class="alignright" value="' . $number_of_people . '" style="width: 0%;" /></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			
			$box_content .= '<p><label for="reservation_date" class="alignleft">' . __( 'Reservation Date:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><input type="text" name="reservation_date" id="reservation_date" class="alignright" value="' . $reservation_date . '" style="width: 100%;" maxlength="10" /><em>(' . __( 'Date format is YYYY-MM-DD-', 'woothemes' ) . ')</em></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			...
			if ( $times_array ) {
				...
				$box_content .= '<span class="right"><select name="reservation_time" class="reservation_time alignleft" size="18">' . "\n";
			...
			*/
			...
			$box_content .= '<p><label for="year" class="alignleft">' . __( 'Car Year:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><input type="text" name="year" id="year" class="alignright" value="' . $year . '" style="width: 100%;" /></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			
			$box_content .= '<p><label for="make" class="alignleft">' . __( 'Car Make:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><input type="text" name="make" id="make" class="alignright" value="' . $make . '" style="width: 100%;" /></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			
			$box_content .= '<p><label for="model" class="alignleft">' . __( 'Car Model:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><input type="text" name="model" id="model" class="alignright" value="' . $model . '" style="width: 100%;" /></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			
			$box_content .= '<p><label for="services" class="alignleft">' . __( 'Services:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><textarea name="services" id="services" class="alignright" rows="5" style="width: 100%;">' . $services . '</textarea></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			
			$box_content .= '<p><label for="services_other" class="alignleft">' . __( 'Services Other:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><textarea name="services_other" id="services_other" class="alignright" rows="5" style="width: 100%;">' . $services_other . '</textarea></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
			
			$box_content .= '<p><label for="additional_notes" class="alignleft">' . __( 'Additional Notes:', 'woothemes' ) . '</label>';
			$box_content .= '<span class="right"><textarea name="additional_notes" id="additional_notes" class="alignright" rows="5" style="width: 100%;">' . $additional_notes . '</textarea></span></p>';
			$box_content .= '<div class="clear"></div><!--/.clear-->';
		...
		public function save_meta_box_reservationdata ( $post_id ) {
			...
			$fields = array( 'number_of_people', 'reservation_date', 'reservation_time', 'year', 'make', 'model', 'services', 'services_other', 'additional_notes', 'contact_tel', 'contact_email', 'reservation_confirmed', 'reservation_status' );
		...
		?>
				<form name="wootable_filter_form" id="wootable_filter_form" action="" method="get">
		    			<select name='wootable_date_filter' id='wootable_date_filter' class='postform alignleft'>
		        			...
		        			<option value="upcoming" <?php if( isset($_GET['wootable_date_filter']) && $_GET['wootable_date_filter']=='upcoming' ) echo 'selected="selected"' ?>><?php _e('Upcoming Appointments','woothemes'); ?></option>
                            <option value="all" <?php if( isset($_GET['wootable_date_filter']) && $_GET['wootable_date_filter']=='all' ) echo 'selected="selected"' ?>><?php _e('All Appointments','woothemes'); ?></option>
		...
		public function posts_where ( $where ) {
		   ...
		   $where .= " AND meta.meta_key = 'reservation_date'";
		    ...
		    if( $_GET['wootable_date_filter'] == '' || $_GET['wootable_date_filter'] == 'upcoming' && is_admin() && $_GET['post_type'] == $this->token ) {
		...
		public static function add_custom_column_headings ( $defaults ) {
			...
			$new_columns['coninfo'] = _x( 'Contact Info', 'column name' );
 			$new_columns['reserveddatetime'] = __( 'Date and Time', 'woothemes' );
			$new_columns['details'] = _x('Vehicle Details', 'column name');
		...
		public static function add_custom_column_data ( $column_name, $id ) {
			...
			switch ($column_name) {
				...
				case 'coninfo':
					...
					$contact_tel = $custom_values['contact_tel'][0];
					$contact_email = $custom_values['contact_email'][0];
					$services = $custom_values['services'][0];
					
					$additional_notes = $custom_values['additional_notes'][0];
					
					echo '<b>' . $contact_tel . '</b>';
					echo '<br>';
					echo $contact_email;
					echo '<br>';
				...
				case 'reserveddatetime':
					...
					$services_other = $custom_values['services_other'][0];
					$reservation_date = $custom_values['reservation_date'][0];
					$date_bits = explode( '-', $reservation_date );
					$date_formatted = date("l, <b>F j</b>, Y", mktime(0, 0, 0, $date_bits[1], $date_bits[2], $date_bits[0] ));
					...
					if ( $is_twelvehour ) {
						...
						$reservation_time = date( '<b>g:i a</b>', mktime( $_hour, $_min, 0, 12, 32, 1997 ) );
						
					} // End IF Statement
					
					if ( strlen( $reservation_time ) ) { $output .= ' <br>' . $reservation_time; } // End IF Statement
					
				
					echo $output;
					if (isset($services_other)) {
						echo '<br><br><b>Other:</b>&nbsp;&nbsp;' . $services_other ;
					} else {
					}
					echo '<br><br>';
				...
				case 'details';
					...
					$year = $custom_values['year'][0];
					$make = $custom_values['make'][0];
					$model = $custom_values['model'][0];
					$additional_notes = $custom_values['additional_notes'][0];
					...
					if (isset($additional_notes)) {
						echo '<br><br><br><b>Notes:</b>&nbsp;&nbsp;' . $additional_notes ;
					} else {
					}
					echo '<br><br>&nbsp;';

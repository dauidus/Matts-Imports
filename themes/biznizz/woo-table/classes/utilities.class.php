	...
	class WooTable_Utilities {
		...
		/*----------------------------------------
	 	...
	 	public function dashboard_widget_setup () {
			...
			$upcoming_reservations_title = __( 'Upcoming Appointments', 'woothemes' );
	 	...
	 	public function dashboard_upcoming_reservations_widget_content () {
				...
				if ( $raw_reservations ) {
					...
					foreach ( $raw_reservations as $r ) {
						...
						$date_formatted = date("l, F j, Y", mktime(0, 0, 0, $date_bits[1], $date_bits[2], $date_bits[0] ));
						...
						$reservation_time = $custom_values['reservation_time'][0];
								...
								if ( $is_twelvehour ) {
									...
									$time_formatted = date( 'g:i a', mktime( $_hour, $_min, 0, 12, 32, 1997 ) );
						...
						$entry = array( 
										...
										'reservation_time' => $time_formatted, 
			...
			?>					
						<div id="comment-<?php echo $r['id']; ?>" class="reservation">
							...
							<div class="dashboard-reservation-wrap">
								<div  class="alignleft">
									<h4><?php echo sprintf( __( ' %s &nbsp;&nbsp;&nbsp; %s &nbsp;&nbsp;&nbsp; %s.', 'woothemes' ), '<h2 style="margin-top:-5px;"><span class="reservation-date">' . $r['reservation_date'] . '</span></h2>', '<h3 style="margin:-20px -100px 0 0;"><span class="reservation-time"><b>' . $r['reservation_time'] . '</b></h3></span><br>', '<span class="reservation-title">' . $r['title'] . '</span>' ); ?></h4>
								
										<h4><?php echo '<span class="reservation-title" style="margin-left:25px;">' . $r['year'] . ' ' . $r['make'] . ', ' . $r['model'] . '</span>' ; ?></h4>
                                        <span class="instructions" style="margin-right:-90px;"><?php echo '<br><b>Services:</b>&nbsp;&nbsp;' . $r['services'] ; ?></span>
                                        <span class="instructions">
											<?php if (isset($r['services_other'])) {
                                            	echo '<br><b>Other:</b>&nbsp;&nbsp;' . $r['services_other'];
											} else {  
                                            } ?>
										</span>
                                        <span class="instructions">
											<?php if (isset($r['additional_notes'])) {
                                            	echo '<br><b>Notes:</b>&nbsp;&nbsp;' . $r['additional_notes'];
											} else {  
                                            } ?>
										</span>
                                        <br />
			...
			?>
				...
				<p class="no-reservations"><?php _e( 'No future appointments have been scheduled.','woothemes' ); ?></p>

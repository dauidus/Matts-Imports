			...
			if ( $business_hours ) {
				...
				if ( $times['closed'] ) {
					...
					if ( isset( $_POST['ajax'] ) && $_POST['ajax'] == 1 && $page_id == 0 ) {
						...
						$_html .= '<span class="reservation_time required">' . sprintf( __( '<br><br><br><Br><br><br>We are closed on %ss.  To inquire about emergency vehicle assistance, contact Matt on his cell at (562) 896-7712.', 'woothemes' ), $full_dayname ) . '<input type="hidden" name="reservation_time" class="required" value="" /></span>' . "\n";
					...
					if ( count( $times_array ) ) {
						...
						$_html .= '<select size="16" name="reservation_time" class="reservation_time required" style="height:360px;">' . "\n";

	 				...
	 				if ( isset( $_email ) && is_email( $_email ) ) {
	 					...
	 					if ( $reservations ) {
							...
							$date_format = 'l, F j, Y';
								...
								$content .= '<table id="wootable-bookings-table">' . "\n";
									...
									$content .= '<tbody>' . "\n";
										...
										foreach ( $reservations as $r ) {
											...
											$posted_time = $timestamp_bits[1]+8;
										...
										if ( $is_twelvehour ) {
											...
											$_reservation_time = date( 'g:i A', mktime( $_hour, $_min, 0, 12, 32, 1997 ) );
										...
										$content .= '<tr>' . "\n";
											...
											$content .= '<td>' . "\n";
											$content .= '<p><small>' . sprintf( __( 'Placed %s%s by %s', 'woothemes' ), '<strong>' . $human_time_diff . '</strong>', '<strong></strong>' , '<strong>' . $r['post_title'] . '</strong>' ) . '</small></p>' . "\n";
											
											$content .= '<p>' . $year . ' ' . $make . ' ' . $model . '</p>' . "\n";
	 						...
	 						$_message = sprintf( __( 'You have no upcoming reservations. To book one, <a href="%s">visit our reservations form</a>.', 'woothemes' ), get_permalink( $this->booking_page ) );
